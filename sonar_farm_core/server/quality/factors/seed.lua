...
local SeedFactor = {}
local FACTOR_NAME = 'seed_quality'
...
local function configured_default()
    ...
    return defaults and defaults.seed_quality or DEFAULT_SCORE
end
...
function SeedFactor:track(plot_id, event, data)
    local db = get_db()
    if not db then
        return false, 'Sonar.Farm.DB is unavailable'
    end

    local score = clamp_score(data and (data.seed_quality or data.score))
    db.execute([[
        INSERT INTO `sonar_farm_quality_tracking` (`plot_id`, `seed_quality`, `calculated_at`)
        VALUES (?, ?, ?)
        ON DUPLICATE KEY UPDATE
            `seed_quality` = VALUES(`seed_quality`),
...
function SeedFactor:get(plot_id)
    local db = get_db()
    if not db then
        return configured_default()
    end

    local row = db.row('SELECT `seed_quality` FROM `sonar_farm_quality_tracking` WHERE `plot_id` = ? LIMIT 1', { plot_id })
    if not row then
        return configured_default()
    end

    return clamp_score(row.seed_quality)
end

Sonar.Farm.Quality.Factors.Seed = SeedFactor

...
local function log_debug(message)
    if is_debug_enabled() then
        print(('[sonar_farm_core][npc_vendor][DEBUG] %s'):format(message))
    end
end

local function log_error(message)
    print(('[sonar_farm_core][npc_vendor][ERROR] %s'):format(message))
end
...
local function ensure_model(model_name)
    ...
    local deadline = GetGameTimer() + (Config.Farm.Client.NPCVendor.ModelLoadDeadlineMs or 5000)
...
local function execute_sale(buyer_id, buyer)
    ...
    local completed = lib.progressBar({
        duration = Config.Farm.Client.NPCVendor.SaleProgressDurationMs or 3000,
    ...
    lib.notify({
        ...
        type = 'success',
        duration = Config.Farm.Client.NPCVendor.SuccessNotifyDurationMs or 5000,
    })
end
...
local function spawn_buyer_ped(buyer_id, buyer)
    ...
    local coords = type(buyer.world_coords) == 'table' and buyer.world_coords or {}
    local x = tonumber(coords.x) or 0.0
    local y = tonumber(coords.y) or 0.0
    local z = tonumber(coords.z) or 0.0
    local heading = tonumber(buyer.heading) or 0.0
    local ped = CreatePed(4, model_hash, x, y, z, heading, false, true)

    if not ped or ped == 0 or not DoesEntityExist(ped) then
        SetModelAsNoLongerNeeded(model_hash)
        log_error(('failed to create buyer ped for %s at %.2f %.2f %.2f'):format(tostring(buyer_id), x, y, z))
        return false
    end
    ...
    log_debug(('spawned buyer %s at %.2f %.2f %.2f heading %.2f'):format(tostring(buyer_id), x, y, z, heading))

    return true
end

local function load_buyers()

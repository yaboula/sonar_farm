...
| **1.5** | 2026-05-22 | §10 Factores de calidad activos en produccion: riego fisico (`sonar_water_tank`, pozos, logica optimo/encharcado), fertilizacion NPK (4 items, curva campana por cultivo, configuracion por-cultivo), plagas (FSM NONE→DETECTED→SEVERE, `PestService`, 2 tipos de plaga, `sonar_pesticide_contact/systemic`, overlay de particula nativa). Migration `011_pest_severity.sql`. Scheduler de plagas reutiliza tick del lifecycle (ADR-018). Estado `pest_severity` como columna explicita (ADR-019). Quality decision surface confirmada: run malo → C/D, run optimo → A/S. Bundle B3 (S13+S14+S15) cerrado. S11 (persistence delta-calc 6h cap) y S12 (maiz + cebada config-only) tambien cerrados con QBox smoke PASS. ADR-016 (timestamps offline), ADR-017 (cierre S11/S12 con QBCore smoke diferido), ADR-018, ADR-019 firmados. Sin cambios en pilares, anti-features, stack ni pricing. |
| **1.4** | 2026-05-22 | §13 Almacen fisico operativo: silo Grapeseed con stash ox_inventory + tabla `sonar_farm_storage_contents` (audit layer, ADR-015). §10 Primer flujo de venta real: Molino Pedro (ped `a_m_m_farmer_01`, coords `1682.15, 4840.8, 42.06`), payout formula `base_price_per_kg x quality_multiplier x weight_kg`, credito en banca via `Sonar.Farm.Finance.Credit`. Migration `008_storage.sql` + `009_finance_amount_decimal.sql`. ADR-015 firmada (post-hook dual-write pattern). QBox smoke PASS; QBCore smoke postponed por founder decision (sin llamadas directas al framework). Sin cambios en pilares, anti-features, stack ni pricing. |

...
| **Tagline operacional** | *Premium-grade farming infrastructure for FiveM.* |
| **Tagline ES** | *La granja que vive cuando no estás.* |
...
| **Inspiración visual** | Linear · Vercel · Arc Browser · Climate FieldView · John Deere Operations Center · laboratorio agrícola premium. **Bento Grid** como paradigma de layout. Glassmorphism cards. |
...
### 1.1 Paleta canónica (a refinar en v0.dev)

| Token | Valor propuesto | Uso |
|---|---|---|
| `--fs-bg` | `#EEF4EB` (mint/sage soft) | Background base de toda surface |
| `--fs-surface` | `rgba(255,255,255,0.7)` + backdrop-blur | Cards Bento, panels |
| `--fs-nav` | `#0A0E0A` (carbón con tinte verde) | Navegación, contraste, sidebar |
| `--fs-accent` | **`#CCFF00`** (verde lima neón) ⭐ | Signal único: CTAs, success states, harvest ready, calidad A/S |
| `--fs-fg` | `#1A1F1A` (carbón) | Texto principal |
| `--fs-fg-muted` | `~#5A625A` (calibrar) | Texto secundario, labels |
| `--fs-warn` | naranja/ámbar (definir) | Warnings (cosecha tardía, plaga detectada) |
| `--fs-error` | rojo suave (definir) | Errores, calidad D, contrato breached |

> **Refinamiento exacto de hex + tokens shadcn se trabajará en v0.dev/Figma** en sesión dedicada de UI. La paleta firmada aquí es la dirección, no los valores finales.
...
> Que cuando un operador FiveM piense en "el script de granja serio", piense en **Farm Sonar**. Que el primer dashboard que vea en su laptop de oficina le haga decir *"esto no parece un script, parece un producto SaaS premium"*.
...
> *"Compras Farm Sonar y te llevas una vertical agrícola completa funcionando como un solo organismo. Cero integración de 5 scripts. Cero debugging de conflictos. Lo enchufas, lo configuras, y tu servidor sube de tier."*

### 2.4 Promesa al operador (jugador final)

> *"Tu granja existe aunque tú no estés. Tus cultivos crecen, tus contratos vencen, tu reputación pesa. Cuando vuelves, el Laptop de la oficina te enseña exactamente qué pasó y qué decisión tomar a continuación. Premium-grade. Bento. Lime neón. Cero fricción."*
...
- Cards modulares de distintos tamaños, glassmorphism translúcido.
- Datos densos pero legibles (mucho aire blanco, jerarquía clara).
- Datos numéricos en **JetBrains Mono**, UI en **Geist Sans**.
- Animaciones sutiles con Framer Motion (transitions ≤300ms, ease-out).
- Lime neón `#CCFF00` como **único color de signal** (CTAs, success states, harvest ready, calidad A/S). Resto en escala carbón/menta/blanco.
...
> **Próxima sesión candidata**: redactar `docs/04_UI_PLAYBOOK.md` (UI canon: paleta hex final firmada, Bento system, motion canon Framer Motion, sound moments, catálogo de componentes, inspiraciones por app, proceso v0.dev) — sesión dedicada con foco aislado.
